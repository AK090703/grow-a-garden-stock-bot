...
def sort_items(category: str, items: List[dict]) -> List[dict]:
    """
    Sort 'items' according to CUSTOM_ORDER[category].
    Unlisted items keep their original feed order and come AFTER your list.
    """
...
def _slug(s: str) -> str:
    """lowercase, strip, collapse non-alnum to single dash for tolerant matching."""
    s = (s or "").strip().lower()
    return re.sub(r"[^a-z0-9]+", "-", s).strip("-")

def _role_candidates(name: str, category: str) -> List[str]:
    """Possible role display names to try, given the item/merchant name + category."""
    pref = {
    ...
    clean = name.strip()
    # Try exact, prefixed, and simple labeled variants
    cands = [
        clean,
...
def _find_role(guild: discord.Guild, display_name: str, category: str) -> Optional[discord.Role]:
    """Return Role whose name matches display_name (tolerant), optionally with category prefix."""
    if not guild or not display_name:
        return None
    cache = _ROLE_CACHE.get(guild.id) or _build_guild_role_cache(guild)
    # try multiple candidate strings -> slug lookup
...
# ===== Parse *_stock payloads + traveling merchant =====
def parse_stock_payload(raw: dict) -> Tuple[Dict[str, List[dict]], Dict[str, Any]]:
    ...
    # Standard "*_stock": seeds, egg, cosmetic, gear, etc.
    for key, items in raw.items():
        if isinstance(key, str) and key.endswith("_stock") and isinstance(items, list):
            base = key[:-6]         # remove "_stock"
            base = base.rstrip("s") # tolerate accidental plurals
    ...
    # Traveling merchant: { merchantName, stock: [...] }
...
# ===== Weather parsing (active only, with icons) =====
def parse_weather_payload(raw: dict) -> List[dict]:
    """
    Returns list of active weathers:
      [{'name': 'Sandstorm', 'remaining': 180, 'end': 1757932920, 'icon': 'https://...'}, ...]
    """
...
# ===== ONE-MESSAGE multi-line output (no icons) for stocks =====
...
async def send_batch_text(category: str, items: List[dict], title_hint: Optional[str] = None):
    ...
    # Apply custom ordering for seeds/pets/gears (others unchanged)
    if category in ("seeds", "pets", "gears"):
        items = sort_items(category, items)

    # ------- COSMETICS: special de-dupe (ignore order) + cooldown (unchanged) -------
    if category == "cosmetics":
        ...
        content = _build_text_lines(category, items, title_hint=title_hint)  # no mentions for cosmetics
        await ch.send(content)
        return
    # -------------------------------------------------------------------------------

    # Other categories (including merchant): keep existing de-dupe on final (possibly sorted) order
    ...
    # Build one message with multiple lines + role mentions where available
    title = f"{category.capitalize()} stock"
    header_mentions: List[str] = []
    roles_to_ping: List[discord.Role] = []

    # For merchant: mention the merchant role in the header (if exists)
    ...
    for it in items:
        ...
        # Mention roles for seeds/pets/gears items (not cosmetics)
...
async def send_weather_embeds(active_weathers: List[dict]):
    """
    Sends ONE message when there are active weathers:
      content: "**Active Weathers (N)**"
      embeds: one per weather, with icon + "Name - 3m 0s left (ends <t:END:R>)"
    If there are no active weathers, do nothing.
    """
    if not active_weathers:
        return  # <-- important: no message when none active
    ...
    # Dedup on name + end timestamp (so we don’t spam)
    ...
    for w in active_weathers[:10]:
        # Include both a static countdown and a Discord relative timestamp that auto-updates
        line = f"{w['name']} - {_fmt_duration(w.get('remaining'))}"
        if w.get("end"):
            line += f" (ends <t:{int(w['end'])}:R>)"
                        ...
                        if msg.type == WSMsgType.TEXT:
                            try:
                                ...
                                if isinstance(raw, dict) and (
                                    ...
                                    # Other categories (apply custom ordering where enabled)
                                ...
                                # 2) Weather (active only, with icons)
...
async def on_ready():
    ...
    for cat, cid in CATEGORY_CHANNELS.items():
        ch = await _resolve_channel(cid)
        if ch:
            try: await ch.send(embed=Embed(title=f"{cat.capitalize()} feed starting…", color=_color(cat)))
            except Exception as e: print(f"[warn] cannot send to {cat}/{cid}: {e}")
        else:
            print(f"[warn] wrong or missing channel ID for {cat}: {cid}")
